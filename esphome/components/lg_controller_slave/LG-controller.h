...
class LgController final : public climate::Climate, public uart::UARTDevice, public Component {
    ...
    void configure_capabilities() {
        ...
        sleep_timer_.set_internal(false);
    }
...
if (is_initializing_) {
    ...
    send_buf_[8] |= 0x40;
    // Set bit 0x40 of byte 9 for Fahrenheit flag if needed
    if (fahrenheit_) {
        send_buf_[9] |= 0x40;
    }
    send_buf_[10] = 0x80;
    ...
    send_buf_[8] = (timer_kind_sleep << 3) | ((*minutes >> 8) & 0x07);
    send_buf_[9] = *minutes & 0xFF;
    send_buf_[3] |= 0x10;
}
else {
    // Proper sleep cancel (THIS WAS MISSING)
    constexpr uint8_t timer_kind_clear_all = 4;
    send_buf_[8] = timer_kind_clear_all << 3;
    send_buf_[9] = 0;
    send_buf_[3] &= ~0x10;
}

// Byte 11: preserve previous value
send_buf_[11] = last_recv_status_[11];

// Byte 12: recalc checksum
...
//Extract sleep timer info safely
    uint8_t timer_kind = (buffer[8] >> 3) & 0x07;               // Bits 3-5
    uint16_t minutes = ((buffer[8] & 0x07) << 8) | buffer[9];   // 11-bit timer

    if (timer_kind == 3) {  // Sleep timer frame
        // Only update if different to prevent redundant HA updates
        if (sleep_timer_.state != minutes) {
            ignore_sleep_timer_callback_ = true;    // block callback to prevent loop
            sleep_timer_.publish_state(minutes);
            ignore_sleep_timer_callback_ = false;
        }
    } else if (!active_reservation_) {
        // AC cleared sleep; reset internal timer if necessary
    ...
    // --- Existing publish state for climate unit ---
    publish_state();
